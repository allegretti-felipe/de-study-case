"""
This module defines a class 'Bolt' that inherits from 'BaseProcessing' and handles the processing of HTML files.
...
class Bolt(BaseProcessing):
    ...
    def run(self) -> None:
        """
        Processes HTML files from the S3 path 'pages/Bolt', extracting specific elements
        ...
        for file in files:
            ...
            texts = self.extract_next_data(tree)
...
if __name__ == "__main__":
    """
    Parses command-line arguments and starts the processing using an instance of the Bolt class.
    """
    ...
    logger.info("Starting Bolt processing...")

    bolt = Bolt(company="Bolt", full_load=args.full_load, debug=args.debug)
    bolt.run()

    logger.info("Finished Bolt processing!")
